...
import { useParams, Link } from "react-router-dom";  // Link -> Works as anchor tags
import { RiShoppingBag3Line } from "react-icons/ri";
import { IoFastFoodOutline, IoNutritionOutline } from "react-icons/io5";
import { BiDrink } from "react-icons/bi";
import classnames from "classnames";

const MobileTabs = () => {  // All the different tabs in navigation menu.
  ...
  const { type } = useParams();  // Extract type from useParams
  ...
  return (
    <>
      <div className="lg:hidden bg-white shadow-lg fixed bottom-0 z-10 w-full flex items-center justify-between md:justify-evenly text-gray-500 border">
        {allTypes.map((item) => (  // Get all items in the list(items)
          <Link key={item.id} to={`/${item.id}`} className="w-1/4"> 
            ...
            >
              ...
              >
                {item.icon}  
                {/* Display icons of the given category. */}
...
const LargeTabs = () => {
  const [allTypes] = useState([
    {
      id: "delivery",
      imageDefault:  // Image when tab is not selected.
        "https://b.zmtcdn.com/data/o2_assets/246bbd71fbba420d5996452be3024d351616150055.png",
      imageActive:  // Image when tab is selected.
        "https://b.zmtcdn.com/data/o2_assets/c0bb85d3a6347b2ec070a8db694588261616149578.png",
      name: "Delivery",
      activeColor: "yellow",  // When tab is selected, bg color.
    },
    {
      id: "dining",
      imageDefault:
          ...
          <Link key={item.id} to={`/${item.id}`} className="w-36">
            <div
              className={classnames(
                "flex items-center gap-3 pb-2 transition duration-700 ease-in-out",  // Default design to be applied whether true or false
                {
                  "border-b-2 border-zomato-400": type === item.id,  // Styles tied to a specific item (item.id in this case)
                }
              )}
            >
              <div
                className={classnames(
                  ...
                  {
                    [`bg-${item.activeColor}-100`]: type === item.id, // bg of active color.
                  }
                )}
              >
                <img
