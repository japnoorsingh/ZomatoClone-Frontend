...
function Overview() {
  ...
  //   map((item) => parseFloat(item)); // [1234324234.343, 23414324214.243] // Converts string into array of floats
  const getLatLong = (mapAddress) => {
    // Map address (Lat = Latitude, Long = Longitude)
  ...
  return (
    <>
      <div className="flex flex-col gap-5 md:flex-row relative">
          ...
          <div className="flex justify-between items-center">
            ...
            <Link to={`/restaurant/${id}/menu`}>
              <span className="flex items-center gap-1 text-zomato-400">
                See all menu items <IoMdArrowDropright />
              </span>
            </Link>
          </div>
        ...
        <aside // Aside tab -> A component which does not follow the normal flow of all other components on the page (Map in this case(only works on large screens (not shown/hidden in small screens)))
